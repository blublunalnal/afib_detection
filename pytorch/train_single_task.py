...
from deepbeat_model import rhythm_only_deepbeat
from utils import (DeepBeatDataset, EarlyStopping, 
                   setup_tensorboard, restore_early_stopping_state, 
                   apply_tuned_params, save_checkpoint, load_checkpoint, load_pickle_file, get_optimal_workers, run_epoch, run_epoch_single_task)
...
def parser_args():
    ...
    parser.add_argument("--test_data_path", default= '/content/ori_test.pkl')
...
def main():
    ...
    test_dataset = DeepBeatDataset(data_test, label_test_q, label_test_r)
    ...
    if resume_checkpoint is not None:
        ...
        if tuned_dropouts is not None:
            model = rhythm_only_deepbeat(dropouts=tuned_dropouts).to(device)
        else:
            model = rhythm_only_deepbeat().to(device)
    ...
    try:
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            writer.add_scalars('Accuracy', {'train': train_m['acc'], 'val': val_m['acc'], 'test': test_m['acc']}, epoch)
           
            
            writer.add_scalars('F1_macro/Rhythm', {'train': train_m['f1'], 'val': val_m['f1'], 'test': test_m['f1']}, epoch)
            writer.add_scalar('Gradients/train', train_m['grad_norm'], epoch)
            
            history['loss'].append(train_m['loss'])
            history['val_loss'].append(val_m['loss'])
            history['val_rhythm_acc'].append(val_m['acc'])
            history['val_rhythm_f1'].append(val_m['f1'])
            
            # Print summary of epoch
            print(f"   -> Train Loss: {train_m['loss']:.4f} | Rh Acc: {train_m['acc']:.4f} | Rh F1: {train_m['f1']:.2f}")
            print(f"   -> Val   Loss: {val_m['loss']:.4f} | Rh Acc: {val_m['acc']:.4f} |Rh f1: {val_m['f1']: .2f}")
