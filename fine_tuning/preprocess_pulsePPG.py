"""
Preprocess DeepBeat data for PulsePPG fine-tuning.

Applies resampling (32Hz -> 50Hz) and z-score normalization once,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Preprocess DeepBeat data for PulsePPG fine-tuning")
...
def preprocess(data, chunk_size=512):
    ...
    Returns:
        np.ndarray of shape (N, C, L') @ 50Hz, z-score normalized
    """
    N        = data.shape[0]
    target_L = int(data.shape[1] * (50 / 32))  # 350
    C        = data.shape[2]
    ...
    for start in range(0, N, chunk_size):
        ...
        chunk = ss.resample_poly(data[start:end], 50, 32, axis=1)  # (chunk, L', C)
